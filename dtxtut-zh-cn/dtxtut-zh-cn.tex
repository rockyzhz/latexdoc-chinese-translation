...
\renewcommand{\clsguide}{面向类文件与宏包编写者的\ \LaTeX}
...
\date{\parbox{\linewidth}{\centering 21 January 2024
...
您还应该熟悉《\clsguide》\footnote{译者注：该文档可对照参阅由本人翻译的\href
{https://github.com/rockyzhz/latexdoc-chinese-translation/blob/main/clsguide-current-zh-cn/clsguide-zh-cn.pdf}{中文版}。}
，它可以在 \CTAN{}
...
\paragraph{术语}
\index{class file|(}
\defn[class file]{类文件 \textnormal{(\clsfile{})}} 指定文档的基本格式：
页面上的文本块大小和位置，结构元素如 \verb|\section| 的排版，页眉和页脚样式等。
一个文档中仅使用一个类文件（通过 \usemacro{documentclass}）。
\index{class file|)}

\index{style file|(}

\defn[style file]{样式文件 \textnormal{(\styfile{})}} 主要是一组宏和环境的定义。
样式文件可以覆盖类文件的格式决策，提供新的功能，或更改现有功能。
一个文档可以加载任意数量的样式文件（通过 \usemacro{usepackage}）。
\index{style file|)}

\index{package|(}
一个或多个类文件或样式文件（例如，一个主样式文件使用 \usemacro{input}
或 \usemacro{RequirePackage} 来加载多个辅助文件）及其文档被称为
\defn{宏包}。在本文档的其余部分，我们使用符号``\m{package}''来表示
你的宏包的名称。{\bf 警告}：在\LaTeX\ 社区中，有时也使用术语``宏包''来指
``样式文件''。您可能需要从上下文中判断使用的是哪个定义。

\paragraph{动机}
一个包\index{package}的重要部分包括代码、代码的文档和用户文档。
使用 \DOC{} 和 \ds{} 程序，可以将这三者合并为一个单一的，
\defn[documented LaTeX file@documented \string\LaTeX{} file]{带文档的 \LaTeX{}
...
这种注重为代码编写详细的、漂亮排版的注释\index{comments}方法——
...
支付任何费用。唯一的限制是他必须为您的工作给予您信用，并且他必须清晰地
将修改过的代码明确标识为修改版本，以避免版本混淆。
\index{package|)}
\index{license|)}
...
\styfile{} 文件中，并在此过程中剥离“|%<<package>>|”。
因此，我们的样式文件 \styfile{} 将在头部注释之后包含以下代码：
...
\index{package|(}
\usemacro{NeedsTeXFormat} 行确保宏包不会在除 \latex[\LaTeXe] 外的
其他\TeX\ 格式下运行。可选的日期参数导致 |latex| 在\latex[\LaTeXe]的版本
（可以通过\latex[\LaTeXe]的 |\fmtversion| 宏找到）早于宏包测试版本时输出警告消息：
...
在 \usemacro{ProvidesPackage} 行中的日期和版本字符串被\DOC{}用于设置
\usemacro{filedate} 和 \usemacro{fileversion} 宏。请注意日期\index{日期格式}
格式；在\latex[\LaTeXe]中，\emph{YYYY-MM-DD} 被广泛使用，因此在您的宏包中也应该使用。
\footnote{\latex[\LaTeXe]的旧版本使用 \emph{YYYY/MM/DD} 格式，但最新的宏包/类作者指南~\cite{LaTeX:clsguide}
规定从现在开始应该使用 \emph{YYYY-MM-DD}。}
...
\begin{enumerate}[itemindent=2em, left= 1.5em .. .5em]
  ...
  \item \usemacro{documentclass} 几乎总是应该使用 \usething{ltxdoc}，
        ...
        \index{package|(}
  \item 你应该总是使用 \usemacro{usepackage} 导入你的样式文件。
        如果不这样做，\DOC{} 将看不到宏包的 \usemacro{ProvidesPackage} 行，
        也就无法设置 \usemacro{filedate} 和 \usemacro{fileversion}
        （参见第~\pageref{code:GetFileInfo} 页）。这也是你应该使用
        \usemacro{usepackage} 导入任何用于排版用户文档的其他宏包的地方
        （与你的宏包需要的宏包不同）。
        \index{package|)}
  ...
  \item \label{item:RecordChanges} 在第~\pageref{code:changes} 页，讨论了如何记录
...
在\latex{}文档中，最常见的是记录宏和环境，\usemacro{NewDocElement} 方便地
用于记录任意宏包组件。它定义了类似于\usemacro{DescribeMacro} 和 \usemacro{DescribeEnv}
的 |\Describe|\m{element-name} 宏。然后，你可以使用
|\begin{|\m{env-name}|}|\dots\linebreak[1]|\end{|\m{env-name}|}|
来标记新元素的实例，类似于描述在\vpageref{code:macro-environment}上的
|\begin{macro}|\dots\linebreak[1]|\end{macro}| 和
|\begin{environment}|\dots\linebreak[1]|\end{environment}|。 \m{options}
参数指定了新元素是 |macrolike|（以反斜杠开头）还是 |envlike|（不以
反斜杠开头），等等。
...
\usemacro{oarg} 用于排版可选参数， \usemacro{parg} 用于排版图形模式参数。
...
包的\index{package}源代码位于\usemacro{MaybeStop}\footnote{在早期版本的\DOC{}中，
\usemacro{MaybeStop}被称为\usemacro{StopEventually}。后者仍然被定义，但已被弃用。}
和\usemacro{Finale}之间。
\usemacro{MaybeStop}带有一个参数，即在代码后要排版的文本块。如果指定了\usemacro{OnlyDescription}
（第~\pageref{desc:OnlyDescription}页），则\usemacro{MaybeStop}之后将不会输出任何内容，
包括\usemacro{Finale}后面的文本。因此，\usemacro{MaybeStop}的\m{text}参数是提供一个文本块的机制，
无论是否排版代码列表，都应该输出该文本块。通常包括参考文献部分和/或以下两个命令之一：
...
中生成变更历史数据。要生成排版的变更历史（|<package>.gls|），用户应该按以下方式运行\usething{makeindex}程序：
...
表示行号。解决方案是对\usemacro{index}命令指定``|usage|''格式，以使页码以斜体排版：

\begin{verbatim}
    \index{explicit indexing|usage}
\end{verbatim}

在 |<package>.dtx| 上运行 |latex| 会在 |<package>.idx| 中生成索引数据。要生成排版的索引（|<package>.ind|），
...
|\begin{macro}|\dotsbrk|\end{macro}| 不是必需的来指示宏定义。它也可以用于指示其它控制序列
的定义，例如计数器、长度和盒子：
...
请注意，上述代码在|\complexMacro|内部定义了|\helperMacro|。文档使用了一个嵌套的|\begin{macro}|来引入
  |\helperMacro|，按照惯例，在外层宏的|\end{macro}|之前放置了一个|\end{macro}|，而不是紧跟在|\helperMacro|的最后的``|}|''后面。
...
\begin{itemize}[itemindent=2em, left= 1.5em .. .5em]
  ...
  \item 一定要阅读《\ds{} 程序》
        ...
        \begin{itemize}
          ...
          \item 从 \dtxfile{} 文件中提取除了 \styfile{} 文件之外的其他内容（例如配置文件或Python代码）。
...
某些包包含 \clsfile{}和 \styfile{}文件。希望从同一个 \dtxfile{}文件中提取这两个文件，并共享相同的
