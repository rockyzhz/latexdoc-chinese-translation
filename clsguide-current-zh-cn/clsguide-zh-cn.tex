...
\title{\color{spot}\huge\bfseries 面向类文件与宏包编写者\\[3pt]的\ \LaTeX\Large\ ——当前版本}
\author{\copyright~版权 2023, \LaTeX\ Project Team.\\
   版权所有%
   \footnote{本文件可根据\LaTeX{}项目公共许可证的条件进行分发和/或修改，可以选择本许可证
   的1.3c版本或（自选）以后的版本 (LPPL v1.3c)。请参阅源文件 \texttt{clsguide.tex} 以获取完整详情。}%
   \\[6pt]
   张泓知\qquad 翻译
}
...
\LaTeX{} 提供了许多命令，旨在帮助您生成结构良好、健壮 (robust) 且可移植的文档类
...
\begin{decl}
   ...
   \arg{definition}
\end{decl}
这个命令与 |\newcommand| 接受相同的参数，但它声明了一个健壮 (robust) 命令，即使 \m{definition} 中的某些代码是脆弱的。
你可以使用此命令来定义新的健壮 (robust) 命令，或重新定义现有命令并使其健壮 (robust) 。如果重新定义了一个命令，会在转录文件中放入日志。
