...
\newCJKfontfamily[hei]\heiti{SourceHanSansSC-regular.otf}[BoldFont=SourceHanSansSC-Bold] %%自定义黑体 \heiti ，在幻灯片中黑体(SimHei)最漂亮
...
\newcommand{\filesection}[1]{\addcontentsline{toc}{subsection}{\hspace{24pt}\sffamily{#1}}\subsection*{\sffamily{#1}}}
\newcommand{\iniTeX}{ini\TeX}

% \setcounter{secnumdepth}{0}

\title{\huge\bfseries\color{spot}\LaTeXe 的配置选项}

\author{\copyright~Copyright 1998, 2001, 2003\ \  \LaTeX\ 项目团队\\[3pt]
   版权所有\\[8pt]赣医一附院神经科\ \ \ 黄旭华\ \ \ \ \ \ \ \ 翻译}
...
\section{{\sffamily 配置} \textbf{\LaTeX}}

新的标准 \LaTeX{}\ 的主要目标之一是通过可靠文档处理系统(reliable document processing system)为所有用户提供自由，该文档处理系统可以连接到高度可移植文档格式(portable document format)，因此其配置(configuration)受到严格限制。\texttt{modguide.tex}\ 文件中的\ 《Modifying \LaTeX{}》\ 【《修改 \LaTeX{}》】一文对此进行了更详细的解释。这样做的一个重要后果是，依赖于任何扩展包(extension package)的任何文档都必须在文档文件中声明此包；这有助于确保文档可在不同的位置(site)工作，该位置 \LaTeX{}\ 系统的配置可能不同。
...
\section{{\sffamily 配置} \textbf{\LaTeX}\ {\sffamily 格式}}
...
虽然我们不鼓励这种自定义(customisation)，但您可以在 \texttt{fntguide.tex}\ 文件中的文档化的源文件(documented source file)\texttt{cmfonts.fdd}\ 和\ 《\LaTeXe{} font selection》【《\LaTeXe{}\ 的字体选择》】中找到有关这些文件的内容(content)及其语法(syntax)的信息。[我们希望在未来的某个时候能够提供关于这个主题(subject)的更多信息和示例]
...
通常，该文件只通过调用 |\ExecuteOptions| 指定默认选项，例如\hfil\break |\ExecuteOptions{dvips}| 或 |\ExecuteOptions{textures}|。
...
\section{\sffamily 非标准版本}

如果你觉得有必要制作一个不同于标准版本(standard version)的 \LaTeX{}\ 版本，而使用上述配置方法是不可能的，那么您应该首先阅读 |modguide.tex| 文件中的\ 《Modifying \LaTeX{}》\ 【《修改 \LaTeX{}》】，这可能会让您意识到您没有这样的需求
